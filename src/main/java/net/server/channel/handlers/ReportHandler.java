        ...
        if (type == 0) {
            if (c.getPlayer().getPossibleReports() > 0) {
                if (c.getPlayer().getMeso() > 299) {
                    ...
                    c.announce(CWvsContext.reportResponse((byte) 4));
                    return;
                }
            } else {
                c.announce(CWvsContext.reportResponse((byte) 2));
                return;
            }
            Server.getInstance().broadcastGMMessage(c.getWorld(), CWvsContext.serverNotice(6, victim + " was reported for: " + description));
            ...
            Server.getInstance().broadcastGMMessage(c.getWorld(), CWvsContext.serverNotice(6, c.getPlayer().getName() + " is probably packet editing. Got unknown report type, which is impossible."));
        }
    }
