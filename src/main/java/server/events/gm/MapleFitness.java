...
public class MapleFitness {
    ...
    public void startFitness() {
        ...
        chr.announce(CField.getClock(900));
        ...
        this.schedulemsg = TimerManager.getInstance().register(() -> {
            ...
            if (chr.getMap().getId() >= 109040000 && chr.getMap().getId() <= 109040004) {
                if (getTimeLeft() > 9000 && getTimeLeft() < 11000) {
                    chr.announce(CWvsContext.serverNotice(0, "You have 10 sec left. Those of you unable to beat the game, we hope you beat it next time! Great job everyone!! See you later~"));
                } else if (getTimeLeft() > 99000 && getTimeLeft() < 101000) {
                    chr.announce(CWvsContext.serverNotice(0, "Alright, you don't have much time remaining. Please hurry up a little!"));
                } else if (getTimeLeft() > 239000 && getTimeLeft() < 241000) {
                    chr.announce(CWvsContext.serverNotice(0, "The 4th stage is the last one for [The Maple Physical Fitness Test]. Please don't give up at the last minute and try your best. The reward is waiting for you at the very top!"));
                } else if (getTimeLeft() > 299000 && getTimeLeft() < 301000) {
                    chr.announce(CWvsContext.serverNotice(0, "The 3rd stage offers traps where you may see them, but you won't be able to step on them. Please be careful of them as you make your way up."));
                } else if (getTimeLeft() > 359000 && getTimeLeft() < 361000) {
                    chr.announce(CWvsContext.serverNotice(0, "For those who have heavy lags, please make sure to move slowly to avoid falling all the way down because of lags."));
                } else if (getTimeLeft() > 499000 && getTimeLeft() < 501000) {
                    chr.announce(CWvsContext.serverNotice(0, "Please remember that if you die during the event, you'll be eliminated from the game. If you're running out of HP, either take a potion or recover HP first before moving on."));
                } else if (getTimeLeft() > 599000 && getTimeLeft() < 601000) {
                    chr.announce(CWvsContext.serverNotice(0, "The most important thing you'll need to know to avoid the bananas thrown by the monkeys is *Timing* Timing is everything in this!"));
                } else if (getTimeLeft() > 659000 && getTimeLeft() < 661000) {
                    chr.announce(CWvsContext.serverNotice(0, "The 2nd stage offers monkeys throwing bananas. Please make sure to avoid them by moving along at just the right timing."));
                } else if (getTimeLeft() > 699000 && getTimeLeft() < 701000) {
                    chr.announce(CWvsContext.serverNotice(0, "Please remember that if you die during the event, you'll be eliminated from the game. You still have plenty of time left, so either take a potion or recover HP first before moving on."));
                } else if (getTimeLeft() > 779000 && getTimeLeft() < 781000) {
                    chr.announce(CWvsContext.serverNotice(0, "Everyone that clears [The Maple Physical Fitness Test] on time will be given an item, regardless of the order of finish, so just relax, take your time, and clear the 4 stages."));
                } else if (getTimeLeft() > 839000 && getTimeLeft() < 841000) {
                    chr.announce(CWvsContext.serverNotice(0, "There may be a heavy lag due to many users at stage 1 all at once. It won't be difficult, so please make sure not to fall down because of heavy lag."));
                } else if (getTimeLeft() > 869000 && getTimeLeft() < 871000) {
                    chr.announce(CWvsContext.serverNotice(0, "[MapleStory Physical Fitness Test] consists of 4 stages, and if you happen to die during the game, you'll be eliminated from the game, so please be careful of that."));
                }
            } else {
                resetTimes();
            }
